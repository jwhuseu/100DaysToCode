...
int main()
{
	...
	for (int TimeCount = 0; TimeCount < UsedTimeStep; TimeCount++)
	{
		for (int n = 1; n < NodeX - 1; n++)
		{
			...
			MomentumCopy[n] = 0.5 * (Momentum[n+1] + Momentum[n-1]) - 0.5 * DeltaTime / DeltaX * ((Density[n+1] * Velocity[n+1] * Velocity[n+1] + Pressure[n+1]) 
	...
	for (int n = 0; n < NodeX; n++)
	{
		if (n == 0)
		{
			cout << " Density || U-Momentum || Total Energy || U-Velocity || Pressure " << "\n";
		}
