 ...
 * 1D Euler equations with the basic 
 * upwind scheme.
 */
...
int main()
{
	...
	for (int TimeCount = 0; TimeCount < UsedTimeStep; TimeCount++)
	{
		...
		for (int n = 1; n < NodeX; n++)
		{
			Residual[n * Equation] = (Solution[n * Variables] * Solution[n * Variables + 4] 
												- Solution[(n - 1) * Variables] * Solution[(n - 1) * Variables + 4]) / DeltaX; //Density equation flux calculation
			Residual[n * Equation + 1] = ((Solution[n * Variables]  * Solution[n * Variables + 4] * Solution[n * Variables + 4] + Solution[n * Variables + 3]) 
													- (Solution[(n-1) * Variables]  * Solution[(n-1) * Variables + 4] * Solution[(n-1) * Variables + 4] + Solution[(n-1) * Variables + 3])) / DeltaX; //U-Momentum equation flux calculation
			Residual[n * Equation + 2] = (Solution[n * Variables + 4] * (Solution[n * Variables + 2] + Solution[n * Variables + 3]) 
													- Solution[(n-1) * Variables + 4] * (Solution[(n-1) * Variables + 2] + Solution[(n-1) * Variables + 3])) / DeltaX; //Energy equation flux calculation
		}
	
		//Solution vector time update
		for (int n = 1; n < NodeX; n++)
		{
			SolutionCopy[n * Variables] = Solution[n * Variables] - DeltaTime * Residual[n * Equation]; //Density
			SolutionCopy[n * Variables + 1] = Solution[n * Variables + 1] - DeltaTime * Residual[n * Equation + 1]; //U-Momentum
			SolutionCopy[n * Variables + 2] = Solution[n * Variables + 2] - DeltaTime * Residual[n * Equation + 2]; //Energy
		}
