...
int main()
{
	...
	for (int TimeCount = 0; TimeCount < UsedTimeStep; TimeCount++)
	{
		...
		{
			Residual[n * Equation] = 0.5 * (Solution[(n+1) * Variables] * Solution[(n+1) * Variables + 4] 
												- Solution[(n - 1) * Variables] * Solution[(n - 1) * Variables + 4]) / DeltaX; //Density equation flux calculation
			Residual[n * Equation + 1] = 0.5 * ((Solution[(n+1) * Variables]  * Solution[(n+1) * Variables + 4] * Solution[(n+1) * Variables + 4] + Solution[(n+1) * Variables + 3]) 
			...
			Residual[n * Equation + 2] = 0.5 * (Solution[(n+1) * Variables + 4] * (Solution[(n+1) * Variables + 2] + Solution[(n+1) * Variables + 3]) 
